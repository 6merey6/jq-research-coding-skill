...
在 notebook 页 iframe 内 `evaluate_script`:
```js
const win = document.getElementById('research').contentWindow;
const nb = win.Jupyter.notebook;
return { connected: nb.kernel ? nb.kernel.is_connected() : false };  // WebSocket 连接状态
```

- `connected=false` → **内核未连接/已断** → 按规则7重启内核(重启会清空全部内存变量，需重新运行该 notebook 的所有定义 cell：基础设施/通用工具/函数定义等，不含触发执行的 cell)
- `connected=true` → 内核连接正常。程序是否在跑由该 cell 的 `In [*]` 提示判断(星号=运行中, 数字=已结束, 见规则5/规则4)；**勿调 `kernel.get_status()`/`kernel.status`**(聚宽定制版 Jupyter 内核对象无此 API, 会报错)
...
> **★重启前确认(防误杀运行中程序)**：先按规则6第0步用 `kernel.is_connected()` 确认确实需要重启。
> - `connected=true` 且该 cell 的 `In [*]` 仍为星号(程序在跑) → **不要重启**(会丢失运行中的取数/执行)，回到规则5继续询问。
> - `connected=false` → 内核确实断了，执行下面重启步骤。
